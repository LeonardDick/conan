...
'''
With these functions the radial velocity of every confined atom at every time step is calculated.
'''


def velocity_calc(inputdict):
    ...
    # Check and correct column names in new_frame
    if 'X' not in new_frame.columns:
        new_frame.rename(columns={'x': 'X', 'y': 'Y', 'z': 'Z'}, inplace=True)

    # Proceed with the adjusted DataFrame
...
def rad_velocity_analysis(inputdict):
    ...
    # Calculate velocity using velocity_calc function
    inputdict = velocity_calc(inputdict)
